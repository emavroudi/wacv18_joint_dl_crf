function fiftysalads_joint_dl_crf(output_dir, output_csv_file, params)
...
format_str = ['joint_dl_crf_e_%d_c_%.3f_lr_%.4f', ...
    ...
    '_skip_%d_win_%d_lambda_%.2f_dict_%d_pca_%d_n_%d_rs_%d_scf_%f_feat_%s'];
...
results_dir_name = sprintf(format_str, nb_epochs, reg_c, learning_rate, ...
    ...
    gradpsi_scaling_factor, feat_type);
...
save(fullfile(results_dir, 'crf_train_log.mat'), 'w', 'psi', ...
    'optimization_log');
...
header_str = ['results_dir,granularity,split,val_acc,val_loss,', ...
    ...
    'nb_components,gradpsi_scaling_factor,feat_type'];
...
row_str = sprintf(row_str_format, results_dir, granularity, split, val_acc, ...
    ...
    use_pca, nb_components, gradpsi_scaling_factor, feat_type);
fprintf(fileID, '%s\n', row_str);
fclose(fileID);
