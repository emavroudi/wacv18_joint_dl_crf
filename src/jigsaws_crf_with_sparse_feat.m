function jigsaws_crf_with_sparse_feat(output_dir, output_csv_file, params)
%% JIGSAWS_CRF_WITH_SPARSE_FEAT: Learning of Conditional Random Field on top
% of sparse codes histograms using Stochastic Gradient Descent
...
results_dir_name = sprintf(format_str, nb_epochs, reg_c, learning_rate, ...
    ...
    random_seed, feat_type, input_type);
...
if strcmp(pairwise_mode, 'pre')
   ...
   %precomputed_pairwise(precomputed_pairwise == 0) = -0.2;
...
header_str = ['results_dir,setup,split,val_acc,val_loss,', ...
              ...
              'nb_components, feat_type, input_type'];
...
row_str = sprintf(row_str_format, results_dir, setup, split, val_acc, ...
                  ...
                  use_pca, nb_components, feat_type, input_type);
fprintf(fileID, '%s\n', row_str);
fclose(fileID);
